...
class ResNet_v12_adv:
    def __init__(self, epochs=40, batch_size=100, load_weights=False, level="32", transfer=False, x_train=None, y_train=None, x_val=None, y_val=None, loss_fn=None, attack_method=None):
        # 'resnet_cifar10' if dataset=='cifar10'  else 'resnet_mnist'
        self.name = 'resnet_'+level+'_fusion_diversity_lessdiv_strongforhigher321v2_avg'
        ...
        self.log_filepath = r'logs/v12_adv_fgsm/'
        self._model = None  # 防止后续出现未定义的情况
        self.wrap_model = None
        self.eps = tf.random_uniform((), 0.01, 0.1)
        # self.eps = 0.0
    ...
    #     y_true = K.reshape(y_true, (-1, self.num_classes, num_model))
    #     y_pred = []
    #     y_pred.append(p3)
    #     y_pred.append(p2)
    #     y_pred.append(p1)
    #     y_pred = K.concatenate(y_pred)
    #     y_pred = K.reshape(y_pred, (-1, self.num_classes, num_model))
    ...
    def log_det(self, p3, p2, p1, y_true_one):
        ...
        y_true.append(y_true_one)
        y_true = K.concatenate(y_true)
        y_true = K.reshape(y_true, (-1, self.num_classes, 3))
        y_pred = []
        y_pred_fornorm = []
        y_pred.append(p3*3)
        y_pred.append(p2*2)
        y_pred.append(p1)
        y_pred_fornorm.append(p3)
        y_pred_fornorm.append(p2)
        y_pred_fornorm.append(p1)
        y_pred = K.concatenate(y_pred)
        y_pred_fornorm = K.concatenate(y_pred_fornorm)
        y_pred = K.reshape(y_pred, (-1, self.num_classes, 3))
        y_pred_fornorm = K.reshape(y_pred_fornorm, (-1, self.num_classes, 3))
        ...
        mask_non_y_pred_fornorm = tf.boolean_mask(
            y_pred_fornorm, bool_R_y_true)

        # batch_size X num_model X (num_class-1), 3-D
        mask_non_y_pred = tf.reshape(
            mask_non_y_pred, [-1, num_model, self.num_classes-1])
        mask_non_y_pred_fornorm = tf.reshape(
            mask_non_y_pred_fornorm, [-1, num_model, self.num_classes-1])

        # batch_size X num_model X (num_class-1), 3-D
        mask_non_y_pred = mask_non_y_pred / \
            tf.norm(mask_non_y_pred_fornorm, axis=2, keepdims=True)

        matrix = tf.matmul(mask_non_y_pred, tf.transpose(mask_non_y_pred, perm=[
                           0, 2, 1]))  # batch_size X num_model X num_model, 3-D
        all_log_det = tf.linalg.logdet(
            matrix+det_offset*tf.expand_dims(tf.eye(num_model), 0))  # batch_size X 1, 1-D
        return all_log_det
    ...
    def custome_loss(self, y_true, y_pred, p3, p2, p1):
        ...
        ce_fusion += K.categorical_crossentropy(y_true, p2)
        ce_fusion += K.categorical_crossentropy(y_true, p1)
        ee = self.Ensemble_Entropy(3*p3, 2*p2, p1)
        log_dets = self.log_det(p3, p2, p1, y_true)
        return ce_final + ce_fusion - lamda * ee - log_det_lamda * log_dets
    ...
    def construct(self):
        ...
        output, p3, p2, p1 = self.residual_network(
        ...
        get_p3 = Model(img_input, self._model.get_layer('activation_36').output)
        get_p2 = Model(img_input, self._model.get_layer('activation_35').output)
        get_p1 = Model(img_input, self._model.get_layer('activation_34').output)
        adv_p3 = get_p3(adv_x)
        adv_p2 = get_p2(adv_x)
        adv_p1 = get_p1(adv_x)
        normal_output = self._model(img_input)
        self.loss_fn = partial(self.adv_loss, adv=adv_output, normal = normal_output, advp3=adv_p3, advp2=adv_p2, advp1=adv_p1, p3=p3, p2=p2, p1=p1)
        self.loss_fn.__name__ = "adv_loss_fd"
        # self._model.summary()

    def adv_loss(self, y_true, y_pred, adv, normal, advp3, advp2, advp1, p3, p2, p1):
        return self.custome_loss(y_true, normal, p3, p2, p1) + self.custome_loss(y_true, adv, advp3, advp2, advp1)
    ...
    def residual_network(self, img_input, classes_num=10, stack_n=5):  # ResNet参数，但不包括weights
        ...
        # 16*16*128
        p1 = Conv2D(128, kernel_size=(3, 3),
                    strides=(2, 2), padding='same')(p1)
        # 8*8*128
        p1 = Conv2D(128, kernel_size=(1, 1),
                    strides=(2, 2), padding='same')(p1)
        ...
        # 8*8*128
        p2 = Conv2D(128, kernel_size=(3, 3),
                    strides=(2, 2), padding='same')(p2)
        ...
        # 8*8*128
        p3 = Conv2D(128, kernel_size=(1, 1),
                    strides=(1, 1), padding='same')(p3)
    ...
    def train(self):
        ...
        checkpoint = ModelCheckpoint('models/'+self.name+'_weight_adv_best_fgsm'+'.h5',  # 这里保存了model
