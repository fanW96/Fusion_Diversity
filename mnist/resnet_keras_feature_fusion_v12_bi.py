...
class ResNet_v12_bi:
    def __init__(self, epochs=40, batch_size=128, load_weights=True,level="32",transfer=False,x_train=None,y_train=None,x_val=None,y_val=None,loss_fn=None,istransfer=False):
        self.name               = 'resnet_'+level+'_fusion_diversity_lessdiv_strongforhigher321v2_avg_bi' #'resnet_cifar10' if dataset=='cifar10'  else 'resnet_mnist'
        ...
        self.log_filepath       = r'logs/v12_bi/'
    ...
    def residual_network(self, img_input,classes_num=10,stack_n=5):  # ResNet参数，但不包括weights
        ...
        p2 = Add()([
            UpSampling2D(size=(2,2), interpolation='bilinear')(p3),
            Conv2D(64, kernel_size=(1,1))(exit2)
        ])
        p1 = Add()([
            UpSampling2D(size=(2,2), interpolation='bilinear')(p2),
            Conv2D(64, kernel_size=(1,1))(exit1)
        ])

        # 16*16*128
        p1 = Conv2D(64, kernel_size=(3, 3), strides=(2, 2), padding='same')(p1)
        # 8*8*128
    ...
    def train(self):          
        ...
        # start training
        self._model.fit(self.x_train, self.y_train, batch_size=self.batch_size,
                            epochs=self.epochs,
                            callbacks=cbks,
                            shuffle=True,
