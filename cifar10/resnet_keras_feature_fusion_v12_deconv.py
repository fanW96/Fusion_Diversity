...
from keras.layers import Conv2D, Conv2DTranspose, Dense, Input, Add, Activation, GlobalAveragePooling2D, BatchNormalization, UpSampling2D, Concatenate, Reshape, Flatten, Average
...
class ResNet_v12_deconv:
    def __init__(self, epochs=120, batch_size=128, load_weights=True,level="32",transfer=False,x_train=None,y_train=None,x_val=None,y_val=None,loss_fn=None,istransfer=False):
        self.name               = 'resnet_'+level+'_fusion_diversity_lessdiv_strongforhigher321v2_avg_deconv' #'resnet_cifar10' if dataset=='cifar10'  else 'resnet_mnist'
        ...
        self.log_filepath       = r'logs/v12_deconv/'
    ...
    def log_det(self, p3, p2, p1, y_true_one):
        ...
        mask_non_y_pred = tf.boolean_mask(y_pred, bool_R_y_true) # batch_size X (num_class-1) X num_models, 1-D
        mask_non_y_pred_fornorm = tf.boolean_mask(y_pred_fornorm, bool_R_y_true)

        mask_non_y_pred = tf.reshape(mask_non_y_pred, [-1, num_model, self.num_classes-1]) # batch_size X num_model X (num_class-1), 3-D
        mask_non_y_pred_fornorm = tf.reshape(mask_non_y_pred_fornorm, [-1, num_model, self.num_classes-1])

        mask_non_y_pred = mask_non_y_pred / tf.norm(mask_non_y_pred_fornorm, axis=2, keepdims=True) # batch_size X num_model X (num_class-1), 3-D
    ...
    def construct(self):
        ...
        output, p3, p2, p1 = self.residual_network(img_input,self.num_classes,self.stack_n)
    ...
    def residual_network(self, img_input,classes_num=10,stack_n=5):  # ResNet参数，但不包括weights
        ...
        p3 = exit3
        p2 = Add()([
            Conv2DTranspose(filters=32,kernel_size=(2,2),strides=(2,2),padding='same')(p3),
            exit2
        ])
        p1 = Add()([
            Conv2DTranspose(filters=16,kernel_size=(2,2),strides=(2,2),padding='same')(p2),
            exit1
        ])

        # 16*16*128
        p1 = Conv2D(32, kernel_size=(3, 3), strides=(2, 2), padding='same')(p1)
        # 8*8*128
        ...
        # 8*8*128
        p3 = Conv2D(128, kernel_size=(3, 3), strides=(1, 1), padding='same')(p3)
    ...
    def train(self):          
        ...
        # set optimizer
#         sgd = optimizers.SGD(lr=0.01, momentum=0.9, nesterov=True) # momentum-动量法,nesterov-NAG
