...
from keras.layers import Conv2D, Dense, Input, Add, Activation, GlobalAveragePooling2D, BatchNormalization, UpSampling2D, Concatenate, Reshape, Flatten, Average, Conv2DTranspose
...
class ResNet_v10:
    def __init__(self, epochs=250, batch_size=128, load_weights=True,level="32",transfer=False,x_train=None,y_train=None,x_val=None,y_val=None,loss_fn=None,istransfer=False):
        self.name               = 'resnet_'+level+'_fusion_diversity_strongforhigher_jacobian_avg' #'resnet_cifar10' if dataset=='cifar10'  else 'resnet_mnist'
    ...
    def jacobian_matrix(self, y_pred, input_x):
        # oringin for logit not softmax
        x = K.reshape(input_x, (-1, self.img_rows*self.img_cols*self.img_channels))
        for i in range(self.num_classes):
            if i == 0:
                jacobian = K.gradients(y_pred[i], input_x)[0]
            else:
                jacobian = K.concatenate([jacobian, K.gradients(y_pred[i], input_x)[0]], axis=0)
        return jacobian

    # def jacobian_loss(self, matrix):
    #     jacobian_sqr = K.square(matrix)
    #     jacobian_sqr_sum = K.sum(jacobian_sqr, axis=np.arange(1, len(jacobian_sqr.shape)))



    def custome_loss(self, y_true, y_pred, p3, p2, p1, input_x):
        lamda = 1
        log_det_lamda = 0.5
        jacobian_loss_lamda = 0.1
        ...
        jacobian_loss = tf.norm(self.jacobian_matrix(y_pred, input_x)) 
        return ce_final + ce_fusion - lamda * ee - log_det_lamda * log_dets + jacobian_loss_lamda * jacobian_loss
    ...
    def construct(self):
        ...
        self.loss_fn = partial(self.custome_loss, p3=p3, p2=p2, p1=p1, input_x=img_input)
    ...
    def residual_network(self, img_input,classes_num=10,stack_n=5):  # ResNet参数，但不包括weights
        ...
        p2 = Add()([
            ...
            Conv2DTranspose(64, kernel_size=(3,3), strides=(2,2), padding='same')(p3),
            Conv2D(64, kernel_size=(1,1))(exit2)
        ])
        p1 = Add()([
            # UpSampling2D(size=(2,2))(p2),
            Conv2DTranspose(64, kernel_size=(3,3), strides=(2,2), padding='same')(p2),
