...
from keras.layers import Conv2D, Dense, Input, Add, Activation, GlobalAveragePooling2D, BatchNormalization, UpSampling2D, Concatenate, Reshape, Flatten
...
class ResNet:
    def __init__(self, epochs=250, batch_size=128, load_weights=True,level="32",transfer=False,x_train=None,y_train=None,x_val=None,y_val=None,loss_fn=None,istransfer=False):
        self.name               = 'resnet_'+level+'_fusion_diversity' #'resnet_cifar10' if dataset=='cifar10'  else 'resnet_mnist'
    ...
    def log_det(self, p3, p2, p1, y_true_one):
        ...
        print(bool_R_y_true.shape, y_pred.shape)
    ...
    def train(self):          
        ...
        checkpoint = ModelCheckpoint('models/'+self.name+'_weight'+'.h5', # 这里保存了model
                monitor='val_loss', verbose=0, save_best_only= True, mode='auto', save_weights_only=True)
